    ...
    public:
        ...
        // DFS to send flow along the path found
        ...
        // Dinic's algorithm for finding max flow
        int solveDinic(int s, int t) {
            int max_flow = 0;
            int level[V];
            int parent[V];

            while (bfs(s, t, level)) { // Construct level graph
                memset(parent, -1, sizeof(parent));
                while (true) {
                    int flow = dfs(s, t, INT_MAX, parent);
                    if (flow == 0) break; // No more augmenting paths
                    max_flow += flow; // Add to the overall max flow
                }
            }
            return max_flow;
        }
        ...
        void minCut(int s, int t) {
            // Step 1: Calculate max flow
            int maxFlow = solveEdmondsKarp(s, t);
            cout << "Max Flow: " << maxFlow << endl;

            // Step 2: Perform BFS to find reachable vertices
            ...
            // Step 3: Print the edges that form the minimum cut
